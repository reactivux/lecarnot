<template>
  <div class="droap_menu profile section-cart">
    <ul v-if="cartStore?.products.length > 0">
      <li class="border-bottom droap_menu_p">
        <h6 style=" display: flex; justify-content: space-between;" class=" title title-h6 droap_menu-title">
          {{ $t("header.dropdowns.carts.title") }} ({{ cartStore?.products.length }})

          <span class="text-end white-color">
            <a href="/panier" class=" white-color ">Voir panier
            </a>
          </span>
        </h6>
    ...
    <ul v-else>
      ...
      <li class="droap_menu_p">
        <p class="text-color">{{ $t("header.dropdowns.carts.description") }}</p>
      </li>
    </ul>
  </div>
  <!-- <PagesStoreTable v-if="isModalSelectTableOpen" v-model="isModalSelectTableOpen"
    @toggleSelectTable="handleToggleSelectTable" /> -->
</template>
<script lang="ts">
...
export default {
  ...
  methods: {
    ...
    showCart() {
      this.isCartOpen.value = !this.isCartOpen.value;
    },
    async checkout() {
